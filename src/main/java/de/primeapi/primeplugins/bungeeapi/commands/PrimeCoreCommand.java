	...
	public void execute(CommandSender commandSender, String[] args) {
		if (args.length == 0) {
			commandSender.sendMessage(
					"PrimeCore(Bungee) v" + PrimeCore.getInstance().getDescription().getVersion() + "!");
			...
			if (args[0].equalsIgnoreCase("update")) {
				...
				if (args.length < 2) {
					...
					p.thePlayer()
					 .sendMessage("§8[§cBungeeAPI§8] §ePluginName §8| §7Aktuelle Version §8| §bNeueste Version ");
					for (RestPlugin plugin : PrimeCore.getInstance().getRestManager().getPlugins()) {
						...
						if (info.isNeverVersion(currVersion)) {
							p.thePlayer()
							 .sendMessage(
									 "§8[§cBungeeAPI§8] §e" + plugin.getName() + " §8| §7" + currVersion + "§8 | §b" + info.getLatest() + " §c§l✖");
						} else {
							p.thePlayer()
							 .sendMessage(
									 "§8[§cBungeeAPI§8] §e" + plugin.getName() + " §8| §7" + currVersion + "§8 | §b" + info.getLatest() + " §a§l✔");
						}
					}
					return;
				}
				if (args[1].equalsIgnoreCase("all")) {
					for (RestPlugin plugin : PrimeCore.getInstance().getRestManager().getPlugins()) {
						...
						try {
							...
							p.thePlayer()
							 .sendMessage(
									 "§8[§cBungeeAPI§8] §e" + plugin.getName() + "§7 wurde §aerfolgreich §7in die " +
											 "Datein §e" + f.getName() + "§7 runtergeladen!");
						} catch (URISyntaxException e) {
							e.printStackTrace();
							p.thePlayer()
							 .sendMessage(
									 "§8[§cBungeeAPI§8] §e" + plugin.getName() + "§7 konnte §cnicht " +
											 "§7herruntergeladen" +
					...
					PrimeCore.getInstance()
					         ...
					         .forEach(plugin -> {
						         p.thePlayer()
						          .sendMessage("§8[§cBungeeAPI§8] §7Installiere §e" + plugin.getName() + "§7...");
						         try {
							         ...
							         p.thePlayer()
							          .sendMessage(
									          "§8[§cBungeeAPI§8] §e" + plugin.getName() + "§7 wurde §aerfolgreich " +
											          "§7in" +
