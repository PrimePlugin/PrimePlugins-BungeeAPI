...
public class OnlineStats {
    ...
    public static DatabaseTask<String> getServer(@NonNull UUID uuid) {
        return new DatabaseTask<>(CompletableFuture.supplyAsync(() -> PrimeCore.getInstance().getDatabase().select("SELECT server FROM prime_bungee_online WHERE uuid = ?")
                .parameters(uuid.toString())
                .getAs(String.class)
    ...
    public static DatabaseTask<Boolean> getAFK(@NonNull UUID uuid) {
        return new DatabaseTask<>(CompletableFuture.supplyAsync(() -> PrimeCore.getInstance().getDatabase().select("SELECT afk FROM prime_bungee_online WHERE uuid = ?")
                .parameters(uuid.toString())
                .getAs(Integer.class)
    ...
    public static DatabaseTask<PlayerParty> getParty(@NonNull UUID uuid) {
        return new DatabaseTask<>(CompletableFuture.supplyAsync(() -> {
            String s = PrimeCore.getInstance().getDatabase().select("SELECT party FROM prime_bungee_online WHERE uuid = ?")
                    .parameters(uuid.toString())
                    .getAs(String.class)
                    .toBlocking().singleOrDefault(null);
